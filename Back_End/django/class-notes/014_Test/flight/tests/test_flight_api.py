...
class FlightTestCase(APITestCase):
  ...
  def test_flight_create_as_non_auth_user(self):
    ...
    # request.user = self.user
    response = FlightView.as_view({'post': 'list'})(request)
  ...
  def test_flight_update_as_staff_user(self): 
    ...
    self.user.is_staff = True
    self.user.save()
    request = self.factory.put('/flight/flights/', data,  HTTP_AUTHORIZATION=f"Token {self.token}")
    response = FlightView.as_view({'put': 'create'})(request)
    self.assertEqual(response.status_code, 201)
